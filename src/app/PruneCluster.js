...
})(PruneCluster2 || (PruneCluster2 = {}));

var PruneCluster2;
(function (PruneCluster2) {})(PruneCluster2 || (PruneCluster2 = {}));

export const PruneCluster = PruneCluster2;

export const PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({// eslint-disable-line no-undef
    initialize: function (size, clusterMargin) {
        ...
        this.Cluster.Project = function (lat, lng) {
            return _this._map.project(new L.LatLng(lat, lng), Math.floor(_this._map.getZoom()));// eslint-disable-line no-undef
        };
        this.Cluster.UnProject = function (x, y) {
            return _this._map.unproject(new L.Point(x, y), Math.floor(_this._map.getZoom()));// eslint-disable-line no-undef
        };
        this._objectsOnMap = [];
    ...
    BuildLeafletCluster: function (cluster, position) {
        var _this = this;
        var m = new L.Marker(position, {// eslint-disable-line no-undef
        ...
        m.on('click', function () {
            ...
            if (b) {
                var bounds = new L.LatLngBounds(new L.LatLng(b.minLat, b.maxLng), new L.LatLng(b.maxLat, b.minLng));// eslint-disable-line no-undef
                var zoomLevelBefore = _this._map.getZoom(), zoomLevelAfter = _this._map.getBoundsZoom(bounds, false, new L.Point(20, 20));// eslint-disable-line no-undef
    ...
    BuildLeafletClusterIcon: function (cluster) {
        ...
        return new L.DivIcon({// eslint-disable-line no-undef
            html: "<div><span>" + cluster.population + "</span></div>",
            className: c,
            iconSize: L.point(iconSize, iconSize)// eslint-disable-line no-undef
        });
    },
    ...
    ProcessView: function () {
        ...
        clusters.forEach(function (cluster) {
            ...
            var position = new L.LatLng(cluster.averagePosition.lat, cluster.averagePosition.lng);// eslint-disable-line no-undef
                    ...
                    for (j = 0, ll = clusterCreationList.length; j < ll; ++j) {
                        ...
                        if (marker._population === 1 && jcluster.population === 1 &&
                            ...
                            if ((marker._population > 1 && jcluster.population > 1) &&
                                ...
                                var poisson = {};
                                L.Util.extend(poisson, jcluster.bounds);// eslint-disable-line no-undef
        ...
        window.setTimeout(function () {
            for (i = 0, l = opacityUpdateList.length; i < l; ++i) {
                ...
                if (m._icon)
                    L.DomUtil.addClass(m._icon, "prunecluster-anim");// eslint-disable-line no-undef
                if (m._shadow)
                    L.DomUtil.addClass(m._shadow, "prunecluster-anim");// eslint-disable-line no-undef
                m.setOpacity(1);
            }
        }, 1);
        if (this._hardMove) {
    ...
    FitBounds: function (withFiltered) {
        ...
        if (bounds) {
            this._map.fitBounds(new L.LatLngBounds(new L.LatLng(bounds.minLat, bounds.maxLng), new L.LatLng(bounds.maxLat, bounds.minLng)));// eslint-disable-line no-undef
        }
    },
    GetMarkers: function () {
...
var PruneClusterLeafletSpiderfier = (L.Layer ? L.Layer : L.Class).extend({// eslint-disable-line no-undef
    _2PI: Math.PI * 2,
    ...
    initialize: function (cluster) {
        ...
        this._multiLines = !!L.multiPolyline;// eslint-disable-line no-undef
        this._lines = this._multiLines ?
            L.multiPolyline([], { weight: 1.5, color: '#222' }) :// eslint-disable-line no-undef
            L.polyline([], { weight: 1.5, color: '#222' });// eslint-disable-line no-undef
    },
    onAdd: function (map) {
        this._map = map;
        ...
        window.setTimeout(function () {
            ...
            var anim = window.setInterval(function () {
                ...
                for (i = 0, l = points.length; i < l; ++i) {
                    ...
                    polylines.push([center, new L.LatLng(center.lat + diffLat * stepRatio, center.lng + diffLng * stepRatio)]);// eslint-disable-line no-undef
                }
    ...
    _generatePointsCircle: function (count, centerPt) {
        ...
        for (i = count - 1; i >= 0; i--) {
            ...
            res[i] = new L.Point(Math.round(centerPt.x + legLength * Math.cos(angle)), Math.round(centerPt.y + legLength * Math.sin(angle)));// eslint-disable-line no-undef
        }
        return res;
    },
